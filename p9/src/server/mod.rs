...
// macOS compatibility: 64-bit types are the default on macOS (no separate *64 variants).
#[cfg(target_os = "macos")]
mod compat {
    ...
    pub unsafe fn fstat64(fd: libc::c_int, buf: *mut libc::stat) -> libc::c_int {
        libc::fstat(fd, buf)
    }
    pub unsafe fn readdir64(dirp: *mut libc::DIR) -> *mut libc::dirent {
        libc::readdir(dirp)
    }
    // Flags not available on macOS — harmless to omit.
    pub const O_DIRECT: libc::c_int = 0;
    pub const O_LARGEFILE: libc::c_int = 0;
    pub const O_NOATIME: libc::c_int = 0;
    // O_PATH doesn't exist on macOS; use O_RDONLY as substitute (opens without write).
    pub const O_PATH: libc::c_int = 0;
    // AT_EMPTY_PATH doesn't exist on macOS.
    pub const AT_EMPTY_PATH: libc::c_int = 0;

    // openat64 → openat. macOS openat is variadic (mode optional), same as Linux.
    pub use libc::openat as openat64;

    pub unsafe fn fstatfs64(fd: libc::c_int, buf: *mut libc::statfs) -> libc::c_int {
        libc::fstatfs(fd, buf)
    }

    // macOS statfs doesn't have f_namelen. Return a reasonable default.
    pub fn statfs_namelen(st: &libc::statfs) -> u32 {
        255 // NAME_MAX on macOS
    }
}
...
// macOS: use compat shims (64-bit types are the default, no *64 variants).
#[cfg(target_os = "macos")]
use self::compat::{
    stat64, fstatat64, fstat64, openat64, fstatfs64,
    O_DIRECT, O_LARGEFILE, O_NOATIME, O_PATH, AT_EMPTY_PATH,
};
// Linux/Android: use libc *64 types directly.
#[cfg(not(target_os = "macos"))]
use libc::{stat64, fstatat64, fstat64, openat64, fstatfs64, O_PATH, AT_EMPTY_PATH};
#[cfg(not(target_os = "macos"))]
const O_DIRECT: libc::c_int = libc::O_DIRECT;
#[cfg(not(target_os = "macos"))]
const O_LARGEFILE: libc::c_int = libc::O_LARGEFILE;
#[cfg(not(target_os = "macos"))]
const O_NOATIME: libc::c_int = libc::O_NOATIME;
...
/// Platform-specific fd path relative to the proc/dev root.
/// Linux: /proc/self/fd/N  →  "self/fd/{fd}"
/// macOS: /dev/fd/N        →  "fd/{fd}"
...
fn statat(d: &File, name: &CStr, flags: libc::c_int) -> io::Result<stat64> {
...
impl Server {
    ...
    pub fn with_config(cfg: Config) -> io::Result<Server> {
        // Open a reference to /proc (Linux) or /dev (macOS) for fd-to-path lookups.
        // Linux: /proc/self/fd/N → file path
        // macOS: /dev/fd/N → file descriptor (no O_PATH, use O_RDONLY)
        #[cfg(not(target_os = "macos"))]
        let proc_path = b"/proc\0";
        #[cfg(target_os = "macos")]
        let proc_path = b"/dev\0";

        let proc_cstr = unsafe { CStr::from_bytes_with_nul_unchecked(proc_path) };
    ...
    fn statfs(&mut self, statfs: &Tstatfs) -> io::Result<Rstatfs> {
        ...
        syscall!(unsafe { fstatfs64(fid.path.as_raw_fd(), buf.as_mut_ptr()) })?;
        ...
        Ok(Rstatfs {
            ...
            #[cfg(target_os = "macos")]
            namelen: 255, // NAME_MAX on macOS (no f_namelen field in statfs)
        })
    }
    ...
    fn set_attr(&mut self, set_attr: &Tsetattr) -> io::Result<()> {
        ...
        if set_attr.valid & P9_SETATTR_MODE != 0 {
            // Safe because this doesn't modify any memory and we check the return value.
            syscall!(unsafe {
                libc::fchmodat(self.proc.as_raw_fd(), path.as_ptr(), set_attr.mode as libc::mode_t, 0)
            })?;
        }
    ...
    fn link(&mut self, link: Tlink) -> io::Result<()> {
        ...
        let path = string_to_cstring(fd_path(target.path.as_raw_fd()))?;
    ...
    fn mkdir(&mut self, mkdir: Tmkdir) -> io::Result<Rmkdir> {
        ...
        // Safe because this doesn't modify any memory and we check the return value.
        syscall!(unsafe { libc::mkdirat(fid.path.as_raw_fd(), mkdir.name.as_ptr(), mkdir.mode as libc::mode_t) })?;
        Ok(Rmkdir {
