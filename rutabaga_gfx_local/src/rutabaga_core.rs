...
impl Rutabaga {
    /// Returns the component type that owns the given resource.
    /// Uses component_mask if exactly one component bit is set, otherwise falls back
    /// to default_component. This enables correct routing when multiple components
    /// coexist (e.g., VirglRenderer + Rutabaga2D).
    fn component_for_resource_id(&self, resource_id: u32) -> RutabagaComponentType {
        self.resources
            .get(&resource_id)
            .and_then(|r| calculate_component(r.component_mask).ok())
    ...
    pub fn create_fence(&mut self, fence: RutabagaFence) -> RutabagaResult<()> {
        if fence.flags & RUTABAGA_FLAG_INFO_RING_IDX != 0 {
            ...
            // For ctx_id=0 fences (2D operations), try the default component first.
            // If it fails (e.g., VirglRenderer in NO_VIRGL mode returns EINVAL for
            // ctx0 fences because vrend is not initialized), fall back to Rutabaga2D
            // which signals immediately (2D ops are synchronous).
            let component = self
                .components
                .get_mut(&self.default_component)
                .ok_or(RutabagaError::InvalidComponent)?;

            if let Err(_) = component.create_fence(fence) {
                if self.default_component != RutabagaComponentType::Rutabaga2D {
                    if let Some(fallback) = self.components.get_mut(&RutabagaComponentType::Rutabaga2D) {
                        fallback.create_fence(fence)?;
                    } else {
                        return Err(RutabagaError::InvalidComponent);
                    }
                } else {
                    return Err(RutabagaError::InvalidComponent);
                }
    ...
    pub fn resource_create_3d(
        ...
        // Try default component first, then fall back to Rutabaga2D.
        // This handles the case where VirglRenderer (Venus-only/NO_VIRGL) is default
        // but can't create 2D resources — Rutabaga2D handles them instead.
        let resource = if let Some(component) = self.components.get_mut(&self.default_component) {
            match component.create_3d(resource_id, resource_create_3d.clone()) {
                Ok(r) => r,
                Err(_) if self.default_component != RutabagaComponentType::Rutabaga2D => {
                    if let Some(fallback) = self.components.get_mut(&RutabagaComponentType::Rutabaga2D) {
                        fallback.create_3d(resource_id, resource_create_3d)?
                    } else {
                        return Err(RutabagaError::InvalidComponent);
                    }
                }
    ...
    pub fn attach_backing(
        ...
        let comp_type = self.component_for_resource_id(resource_id);
        let component = self
            .components
            .get_mut(&comp_type)
            .ok_or(RutabagaError::InvalidComponent)?;
...
impl RutabagaBuilder {
    ...
    pub fn build(mut self) -> RutabagaResult<Rutabaga> {
        ...
        // Always initialize Rutabaga2D — it handles 2D resources (fbcon, cursor)
        // even when a 3D backend (VirglRenderer/Gfxstream) is the default component.
        // Without this, 2D resources fail with InvalidResourceId when Venus/gfxstream
        // is the default.
        {
