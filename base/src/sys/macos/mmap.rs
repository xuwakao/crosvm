...
impl MemoryMapping {
    ...
    unsafe fn try_mmap_populate(
        ...
        let (fd, offset) = match fd {
            Some((fd, offset)) => {
                ...
                let seals = unsafe { libc::fcntl(fd.as_raw_descriptor(), 0 /* F_GET_SEALS not on macOS */) };
                if (seals >= 0) && (seals & 0 /* F_SEAL_WRITE not on macOS */ != 0) {
        ...
        let _ = libc::madvise(addr, size, libc::MADV_FREE);
    ...
    pub fn lock_on_fault(&self, mem_offset: usize, count: usize) -> Result<()> {
        ...
        let ret = unsafe { libc::mlock(addr as *mut _, count) };
        if ret < 0 {
            let errno = crate::errno::Error::last();
            warn!(
...
mod tests {
    ...
    fn from_fd_offset_invalid() {
        ...
        let res =
            MemoryMapping::from_fd_offset(&fd, 4096, (libc::off_t::MAX as u64) + 1).unwrap_err();
        match res {
            Error::InvalidOffset => {}
    ...
    fn arena_madvise() {
        ...
        let ps = pagesize();
        <dyn MappedRegion>::madvise(&m, 0, ps, libc::MADV_FREE).unwrap();
        <dyn MappedRegion>::madvise(&m, 0, size, libc::MADV_FREE).unwrap();
        <dyn MappedRegion>::madvise(&m, ps, size - ps, libc::MADV_FREE).unwrap();
        let res = <dyn MappedRegion>::madvise(&m, ps, size, libc::MADV_FREE).unwrap_err();
        match res {
            Error::InvalidAddress => {}
            e => panic!("unexpected error: {e}"),
        }
